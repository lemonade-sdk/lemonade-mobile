...
/// • Already subscribed → change the AI tier in place (`/billing/change-plan`)
///   and add/remove add-ons in place (`/billing/add-package` ·
///   `/billing/remove-package`), all with immediate proration; `/billing/portal`
///   remains for cancel / payment method.
class SubSheet extends ConsumerStatefulWidget {
  final String section; // 'plan' | 'billing'
  const SubSheet({super.key, required this.section});

  @override
  ConsumerState<SubSheet> createState() => _SubSheetState();
}

class _SubSheetState extends ConsumerState<SubSheet> {
  ...
  bool _visibleFor(String audience, String? segment) =>
      audience == 'Both' || segment == null || audience == segment;
